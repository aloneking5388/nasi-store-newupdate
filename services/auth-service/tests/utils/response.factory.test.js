...
const { loginSuccess, notFound, unauthorized, serverError } = require('../../src/modules/auth/utils/response.factory');
...
test('loginSuccess body contains success, message, userInfo, and token', () => {
  ...
  const r = loginSuccess('Customer Login successfully', userInfo, 'mytoken', ['cookie-str']);
...
test('error message strings are preserved exactly, not normalized', () => {
  ...
  assert.equal(unauthorized('Invalid credentials').body.error, 'Invalid credentials');
