...
} = require("./modules/auth/controllers/customer-login.controller");
const {
  logoutController,
} = require("./modules/auth/controllers/logout.controller");
...
const server = http.createServer(async (req, res) => {
  ...
  if (
    req.method === "POST" &&
    parsedUrl.pathname === "/api/auth/customer/login"
  ) {
    try {
...
server.listen(port, host, () => {
  console.log(
    `auth-service listening on http://${host === "0.0.0.0" ? "localhost" : host}:${port}`,
  );
